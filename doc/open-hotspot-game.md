...
The exact scale is subject to optimization. Each participant within a given contest is eligible to receive the following:
...
The following will only be granted once per AP per contest and to the contestant who verifies the respective aspect first:
...
* Only legal and ethical methods allowed: e.g., no networks which would require a password to associate with or that do not broadcast their network name, no sidestepping of portal login pages where authorization is required
